...
pro weighted_combine_bak,obj_id
...
;	ycen = dither pattern
	...
	BF_in = path+'Bf01_sky_res.fits'
	RF_in = path+'Rf01_sky_res.fits'
	...
	for ni=0,n_exp(0)-1 do begin
		BF_in = path+'Bf'+string(ni+1,format='(I02)')+'_sky_res.fits'
		BN_in = path+'BN'+string(ni+1,format='(I02)')+'.fits'
		RF_in = path+'Rf'+string(ni+1,format='(I02)')+'_sky_res.fits'
